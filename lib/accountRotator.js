...
class AccountRotator {
    ...
    getNextSequential() {
        ...
        // Находим текущий username бота в списке
        const currentUsername = this.bot.username;
        let currentIndexInList = -1;

        for (let i = 0; i < this.accounts.length; i++) {
            if (this.accounts[i].username === currentUsername) {
                currentIndexInList = i;
                break;
            }
        }

        // Если текущий username найден, берём следующий
        // Иначе начинаем с первого
        if (currentIndexInList !== -1) {
            this.currentIndex = (currentIndexInList + 1) % this.accounts.length;
        } else {
            this.currentIndex = 0;
        }
    ...
    async changeAccount(order, maxAttempts = null) {
        ...
        for (let i = 0; i < attempts; i++) {
            ...
            // Пропускаем если это тот же аккаунт
            if (account.username === currentUsername) {
                this.log(`[AccountRotator] Пропуск: ${account.username} уже используется`);
                continue;
            }
